...
func (s *Server) followerLoop() {
	...
	s.setState(Follower)
	leftTime := randBetween(s.ElectionTimeout(), s.ElectionTimeout()*2)
	startTime := time.Now()
	...
	for {
		...
		// other cases we should count down the leftTime
		if update {
			leftTime = randBetween(s.ElectionTimeout(), s.ElectionTimeout()*2)
		} else {
			spendTime := time.Now().Sub(startTime)
			if spendTime < leftTime {
				leftTime = leftTime - spendTime
			} else {
				s.setState(Candidate)
			}
		}
