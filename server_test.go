...
func TestServerRequestVoteDenyIfCandidateLogIsBehind(t *testing.T) {
	tmpLog := newLog()
	e0, _ := newLogEntry(tmpLog, newCommandEvent(&testCommand1{Val: "foo", I: 20}), 1, 1)
	e1, _ := newLogEntry(tmpLog, newCommandEvent(&testCommand2{X: 100}), 2, 1)
	e2, _ := newLogEntry(tmpLog, newCommandEvent(&testCommand1{Val: "bar", I: 0}), 3, 2)
...
func TestServerPromoteSelf(t *testing.T) {
	e0, _ := newLogEntry(newLog(), newCommandEvent(&testCommand1{Val: "foo", I: 20}), 1, 1)
...
func TestServerAppendEntries(t *testing.T) {
	...
	// Append single entry.
	e, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "foo", I: 10}), 1, 1)
	entries := []*LogEntry{e}
	...
	e1, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "bar", I: 20}), 2, 1)
	e2, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "baz", I: 30}), 3, 1)
...
func TestServerAppendEntriesRejectedIfAlreadyCommitted(t *testing.T) {
	...
	e1, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "foo", I: 10}), 1, 1)
	e2, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "foo", I: 15}), 2, 1)
	entries := []*LogEntry{e1, e2}
	...
	e, _ := newLogEntry(nil, newCommandEvent(&testCommand1{Val: "bar", I: 20}), 2, 1)
	entries = []*LogEntry{e}
...
func TestServerAppendEntriesOverwritesUncommittedEntries(t *testing.T) {
	...
	entry1, _ := newLogEntry(s.(*server).log, newCommandEvent(&testCommand1{Val: "foo", I: 10}), 1, 1)
	entry2, _ := newLogEntry(s.(*server).log, newCommandEvent(&testCommand1{Val: "foo", I: 15}), 2, 1)
	entry3, _ := newLogEntry(s.(*server).log, newCommandEvent(&testCommand1{Val: "bar", I: 20}), 2, 2)
