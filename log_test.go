...
func TestLogNewLog(t *testing.T) {
	...
	e, _ := newLogEntry(log, newCommandEvent(&testCommand1{Val: "foo", I: 20}), 1, 1)
	if err := log.appendEntry(e); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
	e, _ = newLogEntry(log, newCommandEvent(&testCommand2{X: 100}), 2, 1)
	if err := log.appendEntry(e); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
	e, _ = newLogEntry(log, newCommandEvent(&testCommand1{Val: "bar", I: 0}), 3, 2)
	if err := log.appendEntry(e); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
...
func TestLogRecovery(t *testing.T) {
	...
	e, _ := newLogEntry(log, newCommandEvent(&testCommand1{Val: "bat", I: -5}), 3, 2)
	if err := log.appendEntry(e); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
...
func TestLogTruncate(t *testing.T) {
	...
	entry1, _ := newLogEntry(log, newCommandEvent(&testCommand1{Val: "foo", I: 20}), 1, 1)
	if err := log.appendEntry(entry1); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
	entry2, _ := newLogEntry(log, newCommandEvent(&testCommand2{X: 100}), 2, 1)
	if err := log.appendEntry(entry2); err != nil {
		t.Fatalf("Unable to append: %v", err)
	}
	entry3, _ := newLogEntry(log, newCommandEvent(&testCommand1{Val: "bar", I: 0}), 3, 2)
